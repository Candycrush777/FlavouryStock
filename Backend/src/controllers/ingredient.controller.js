...
exports.registerBasket2 = (req, res) => {
  //todo original sin fechas calculadas
      ...
      db.query(
        ...
        (err, result) => {
          if (err) {
            ...
            return res
              .status(404)
              .json({ err: "No se han insertado los datos en Etiqueta" });
          } else {
            res
              .status(200)
              .json({ message: ` Etiquetas generadas en BD correctamente` });
          }
        }
      );
    }
...
exports.registerBasket = async (req, res) => {
  try {
    ...
    const lugaresInvalidos = []; // ← Aquí recolectamos los lugares con caducidad NULL
    ...
    const insertarEtiquetaFunct = (lugar_almacen, cantidad, diasCaducidad) => {
      if (cantidad > 0 && diasCaducidad !== null && diasCaducidad !== undefined) {
        ...
        lugaresInvalidos.push(lugar_almacen); // ← Guardamos el lugar no válido
      }
    };
    ...
    res.status(200).json({ message, lugaresInvalidos }); // ← Enviamos los lugares inválidos
  } catch (error) {
  ...
  try {
    ...
    const insertarEtiquetaFunct = (lugar_almacen, cantidad, diasCaducidad) => {
      ...
      if (cantidad > 0) {
        ...
        console.log(
          `Insertando ${cantidad} en ${lugar_almacen} con diasCaducidad=${dias}`
        );
        ...
        db.query(
          ...
          (err, result) => {
            if (err) {
              console.error(
                `Error al insertar etiqueta de ${lugar_almacen}:`,
                err
              );
              return res.status(500).json({ error: err.message });
            }
    ...
    insertarEtiquetaFunct("almacen", cantidad_almacen, caducidades.almacen);
    insertarEtiquetaFunct("nevera", cantidad_nevera, caducidades.nevera);
    insertarEtiquetaFunct(
      "congelador",
      cantidad_congelador,
      caducidades.congelador
    );
