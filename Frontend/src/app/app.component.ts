...
export class AppComponent {
  ...
  constructor(private router: Router) {
    ...
    router.events
      ...
      .subscribe(() => {
        this.changeHeader();
        const isErrorRoutes =
          this.router.routerState.snapshot.root.firstChild?.component ===
          NotFoundComponent;
  ...
  changeHeader() {
    const user =
      typeof window !== 'undefined' ? localStorage.getItem('id_rol') : null; //esto es para chequear que localStorage este disponible
    if (user === '1' || user === '2') {
